...
describe('sumTotalsDollarFormatter', () => {
  it('should display an empty string when no value is provided', () => {
    const output = sumTotalsDollarFormatter({}, {} as Column);
    expect(output).toBe('');
  });
  ...
  it('should display an empty string when the "sum" does not find the field property in its object', () => {
    ...
    const output = sumTotalsDollarFormatter(totals, columnDef, {});
    expect(output).toBe('');
  });
  ...
  it('should display a negative sum in red with at least 2 decimals in parentheses instead of the negative sign when its input is negative', () => {
    ...
    const output1 = sumTotalsDollarFormatter(totals, { id: 'column1', field: 'column1', params: { displayNegativeWithParentheses: true } } as Column, {});
    const output2 = sumTotalsDollarFormatter(totals, { id: 'column2', field: 'column2', params: { maxDecimal: 2, displayNegativeWithParentheses: true } } as Column, {});
  ...
  it('should display a sum number in correct color with at least 2 decimals when user provided minimum & maximum decimal count', () => {
    ...
    const output3 = sumTotalsDollarFormatter(totals, { id: 'column3', field: 'column3', params: { minDecimal: 3, displayNegativeWithParentheses: true } } as Column, {});
