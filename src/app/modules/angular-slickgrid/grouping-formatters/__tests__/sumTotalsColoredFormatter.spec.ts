...
describe('sumTotalsColoredFormatter', () => {
  ...
  it('should display a negative sum in red with parentheses instead of the negative sign when its input is negative', () => {
    ...
    const output1 = sumTotalsColoredFormatter(totals, { id: 'column1', field: 'column1', params: { displayNegativeWithParentheses: true } } as Column, {});
    const output2 = sumTotalsColoredFormatter(totals, { id: 'column2', field: 'column2', params: { maxDecimal: 2, displayNegativeWithParentheses: true } } as Column, {});

    expect(output1).toBe('<span style="color:red">(123)</span>');
    expect(output2).toBe('<span style="color:red">(34.57)</span>');
  });

  it('should display a positive green sum number even when displayNegativeWithParentheses is enabled', () => {
    const totals = { sum: { column1: 123, column2: 34.5678, column3: 2.4 } };
  ...
  it('should display a sum number in green (or red for negative) with user defined minimum & maximum decimal count', () => {
    ...
    const output3 = sumTotalsColoredFormatter(totals, { id: 'column3', field: 'column3', params: { minDecimal: 3, displayNegativeWithParentheses: true } } as Column, {});
